...
# add other directory dependencies
sys_paths = ['../']
for p in sys_paths:
    p = os.path.abspath(p)
...
FREQUENCY = 10
IMG_WIDTH = 640 # from ROSbot camera launch files
IMG_HEIGHT = 480
FOV = 60 # camera field of view

class ObjectDetection:
    def __init__(self): # adding MS COCO classes
        ...
        self.model = Yolov3Tiny(num_classes=len(self.class_names)) # implementation from https://github.com/holli/yolov3_pytorch
        self.model.load_state_dict(torch.load('../yolov3_pytorch/yolov3_tiny_coco_01.h5')) # weights based on https://pjreddie.com/darknet/yolo/ 

    # http://wiki.ros.org/rospy_tutorials/Tutorials/WritingImagePublisherSubscriber
    # https://github.com/holli/yolov3_pytorch
    def _image_callback(self, msg):
        np_arr = np.fromstring(msg.data, np.uint8) # convert compressed image to rgb image
        ...
        img_torch = image2torch(img_resized) # conver to tensor

        all_boxes = self.model.predict_img(img_torch, conf_thresh=0.2)[0] # get preds
        ...
        if len(b) > 0:
            ...
            boxes[:, 0] *= IMG_WIDTH # draw bounding boxes dims
            boxes[:, 2] *= IMG_WIDTH
            boxes[:, 1] *= IMG_HEIGHT
            boxes[:, 3] *= IMG_HEIGHT
            ...
            for i in range(len(boxes)):
                ...
                if b[0] == 0:
                    break
                if self.class_names[classes[i]] == 'clock': # object of interest
                    ...
                    bb_bottom = (x + w/2, y)
                    rel_angle = np.radians((FOV/(IMG_WIDTH) * bb_bottom[0] - FOV/2) * -1) # angle adjustment computation
                    self.rel_angle = rel_angle

            plot_img_detections(img_resized, boxes_found, figsize=(8,8), class_names=self.class_names) # plot to viz
    ...
    def ssh_scp_files(self, ssh_host, ssh_user, ssh_password, ssh_port, source_volume, destination_volume):
        ...
        with SCPClient(ssh.get_transport()) as scp:
            scp.put(source_volume, recursive=True, remote_path=destination_volume) # copy over via scp b/c publishing to robot restricted
        ...
        while not rospy.is_shutdown():
            if self.rel_angle:
                with open('./object_angle.txt', 'w') as f:
                    f.write(str(self.rel_angle) + ' ' + str(self.timestamp_secs) + ' ' + str(self.timestamp_nsecs)) # write sending file and send
...
if __name__ == "__main__":
    rospy.init_node('object_detection') # create node
