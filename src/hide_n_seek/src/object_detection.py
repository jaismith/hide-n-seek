...
class ObjectDetection:
    def __init__(self):
        ...
        # self._angle_pub = rospy.Publisher('object_angle', Float32, queue_size=1)

        self._rel_angle = None
    ...
    def _image_callback(self, msg):
        ...
        if len(b) > 0:
            ...
            for i in range(len(boxes)):
                ...
                print(self.class_names[classes[i]])
                # print(class_id)
                ...
                if self.class_names[classes[i]] == 'clock':
                    ...
                    bb_bottom = (x + w/2, y)
                    # if bb_bottom[0] < IMG_WIDTH/2:
                    rel_angle = np.radians((FOV/(IMG_WIDTH) * bb_bottom[0] - FOV/2) * -1)
                    # img_center = (IMG_WIDTH/2, IMG_HEIGHT/2)
                    # rel_angle = math.atan2(bb_bottom[1] - img_center[1], bb_bottom[0] - img_center[0])
                    # print(rel_angle)
    ...
    def detect(self):
        ...
        while not rospy.is_shutdown():
            if self.rel_angle:
                # msg = Float32()
                # msg.data = np.float32(self.rel_angle)
                with open('./object_angle.txt', 'w') as f:
                    f.write(str(self.rel_angle))
